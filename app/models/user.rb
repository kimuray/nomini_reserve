class User < ApplicationRecord
  ...
  # Validation
  validates :l_name_kana, presence: true, format: { with: /\A[ァ-ンー－]+\z/, message: "フルネームは全角カタカナで入力してください。"}, on: :member_create
  validates :f_name_kana, presence: true, format: { with: /\A[ァ-ンー－]+\z/, message: "フルネームは全角カタカナで入力してください。"}, on: :member_create
  validates :phone_number, presence: true, format: { with: /\A[0-9]+\z/, message: "電話番号は数字のみで入力してください。"}, on: :member_create
